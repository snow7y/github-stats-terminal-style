...
async function main() {
  ...
  const aboutFileName = (process.env.ABOUT_FILE || "about.py").trim();
  const aboutFilePath = path.resolve(process.cwd(), aboutFileName);
  const aboutFileContent = loadAboutFile(aboutFilePath);
  ...
  try {
    ...
    terminalService.addCommand("exit", "exit");
...
function loadAboutFile(filePath: string): string {
  try {
